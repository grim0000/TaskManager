...
def query_groq(prompt: str) -> dict:
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "You are a task management assistant. Return a valid JSON object with no comments or explanatory text, let the query be long but parse it according to the instructions: {description: string, due_date: string (YYYY-MM-DD or relative like 'tomorrow'), tags: string[], priority: string (Urgent, Very high, High, Medium, Low, Very low), insights: string, category: string}."},
